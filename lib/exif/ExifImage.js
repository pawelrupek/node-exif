...
function ExifImage (options, callback) {
  ...
  } else if (options instanceof Buffer || options instanceof ArrayBuffer) {
    options= {
        image: options
    }
  }
...
//    callback(new Error('You have to provide an image, it is pretty hard to extract Exif data from nothing...'));
  ...
  // https://github.com/gomfunkel/node-exif/issues/77
  var self=this;
  function _loadImg() {
...
ExifImage.prototype.loadImage = function (image, callback) {
  ...
  if (image.constructor.name === 'ArrayBuffer') {
    let b = Buffer.from(image);
    this.processImage("Buffer", b, callback);
    return;
  }
